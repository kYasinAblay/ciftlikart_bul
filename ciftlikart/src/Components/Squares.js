...
export default class Squares extends Component {
                ...
                if (Object.keys(pair).length > 1) {
                    ...
                    if (!equalSquare) {
                        for (let item of pair) {
                            this.doWork("Slowmotion invisible job", 
                    ...
                    // this.setState({
                    //     pair:  this.state.pair.filter(value =>
                    //         !contents.includes(value.spanElement.textContent)
                    //     )
                    // });
                }
            }
    ...
    Squares = () => {
        ...
        var squaresDiv = numbers.map((value, index) => {
            return (
                <div key={index} className="square" onClick={async (e) => await this.ShowNumber(e)}><span key={index} className="invisible">{value}</span></div>)
        });
        return squaresDiv;
    }
